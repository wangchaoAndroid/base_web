 ...
 */
package core.com.eryansky.common.excel;
...
import core.com.eryansky.common.excel.annotation.Excel;
...
public class ExportExcel<T> {
	 ...
	 */
	public HSSFWorkbook  exportExcel(String title, Class<T> pojoClass,
			Collection<T> dataSet,String filter) {
		// 使用userModel模式实现的，当excel文档出现10万级别的大数据文件可能导致OOM内存溢出
		return exportExcelInUserModel2File(title, pojoClass, dataSet,filter);
	}
	//设置过滤的列
	private HSSFWorkbook exportExcelInUserModel2File(String title, Class<T> pojoClass,
			Collection<T> dataSet,String filter) {
		...
		try {
			...
			for (int i = 0; i < fileds.length; i++) {
				...
				if (excel != null) {
					//过滤参数中的列
					if(filter.indexOf(excel.exportName())!=-1)
						continue;
					// 添加到标题
					exportFieldTitle.add(excel.exportName());
					// 添加标题的列宽
