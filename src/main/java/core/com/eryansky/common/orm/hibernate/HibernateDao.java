 ...
 */
package core.com.eryansky.common.orm.hibernate;
...
import core.com.eryansky.common.orm.PropertyFilter.MatchType;
...
//            Long count = (Long)createQuery(countHql, parameter).uniqueResult();
	...
	protected long countCriteriaResult(final Criteria c) {
		...
		//criteriaImplStrengthen(impl);//使用关联查询
		...
		// 执行Count查询
		Long totalCountObject = (Long) c.setProjection(Projections.rowCount()).setCacheable(true)
				//改变
				.uniqueResult();
	...
	protected Criterion buildCriterion(final String propertyName,
		...
		case GT:
            ...
            break;
		case IN:
			String[] strs = propertyValue.toString().split(",");
			criterion = Restrictions.in(propertyName, strs);
			break;
		case SQL:
			criterion = Restrictions.sqlRestriction(propertyName, propertyValue, StringType.INSTANCE);
            break;
        case ISNULL:
     ...
     */
    @SuppressWarnings("unchecked")
    public <E> Page<E> findBySql1(Page<E> page, String sqlString, Parameter parameter, Class<?> resultClass) {
        // get count
        String countSqlString = "select count(*) " + super.removeSelect(super.removeOrders(super.removeGroups(sqlString)));
        // page.setCount(Long.valueOf(createSqlQuery(countSqlString, parameter).uniqueResult().toString()));
        Query countQuery = createSqlQuery(countSqlString, parameter);
        List<Object> list = countQuery.list();
        if (list.size() > 0) {
        ...
        query.setFirstResult(page.getFirstResult());
        query.setMaxResults(page.getMaxResults());
        page.setResult(query.setResultTransformer(Transformers.aliasToBean(resultClass)).list());
        return page;
    }
    //---------------------
	/**
	 ...
	 */
	@SuppressWarnings({ "unchecked"})
	public Page<T> findPage(final Page<T> page,Map<String,String> map, final Criterion... criterions) {
		Assert.notNull(page, "page不能为空");

		Criteria c = createCriteria(criterions);
		
		for (String key : map.keySet()) 
			c.createAlias(key, map.get(key),1);  //可以指定使用left out  join..，可以改 
		...
		setPageParameterToCriteria(c, page);
		List<T> result = c.setCacheable(true).list();
		//List result = c.list();  改变
		page.setResult(result);
		return page;
	}
	/**
	 * 按属性过滤条件列表分页查找对象.支持多表关联
	 */
	public Page<T> findPageJoin(final Page<T> page,final List<PropertyFilter> filters) {
    	Map<String,String>  map=new HashMap<String,String>();
    	map=result(filters);
		Criterion[] criterions = buildCriterionByPropertyFilter(filters);	
		return findPage(page,map,criterions);
	}
   public  Map<String,String>  format(final Map<String,String> map, String st)
   { 
	   ...
	   {
		   map.put(strs[0],strs[0]);
		   return map;
	   }
	   else if(length==3)
	   {
		   map.put(strs[0]+"."+strs[1],strs[1]);
		   map.put(strs[0],strs[0]);
		   return map;
	   }
	   else 
	   {   
		   map.put(strs[1],strs[0]+","+strs[1]);
		   String temp=st.replace(strs[0]+".","").replace(strs[1]+".","");
		   return format(map,temp);
	   }
   }
   public   Map<String,String> result (List<PropertyFilter> filters)
   { 
	   Map<String,String> map=new HashMap<String,String>();
	   for(PropertyFilter propertyFilter:filters)
	   {
		   for(int i=0;i<propertyFilter.getPropertyNames().length;i++)
		   {
			   map=format(map,propertyFilter.getPropertyNames()[i]);
			   String strs[]=propertyFilter.getPropertyNames()[i].split("\\.");
			   if(strs.length>2)
				   propertyFilter.getPropertyNames()[i]=(strs[strs.length-2]+"."+strs[strs.length-1]);
		   }
	   }
	   return map;
   }
   ...
   /**
	 * 按属性过滤条件列表查找对象列表.
	 */
	public List<T> findJoin(List<PropertyFilter> filters) {
		Criterion[] criterions = buildCriterionByPropertyFilter(filters);
		Map<String,String>  map=new HashMap<String,String>();
    	map=result(filters);
		return findjoin(map,criterions);
	}
  }