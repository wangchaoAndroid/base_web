...
import core.com.eryansky.common.orm.annotation.Delete;
...
public class SimpleHibernateDao<T, PK extends Serializable> {
	...
	public T findUniqueBy(final String propertyName, final Object value) {
		Assert.hasText(propertyName, "propertyName不能为空");
		Criterion criterion = Restrictions.eq(propertyName, value);
		return (T) createCriteria(criterion).setCacheable(true).uniqueResult();
		//改变
	}
		...
		for (Criterion c : criterions) {
			...
			if(str.indexOf("_cnm")!=-1){
				...
				for(int j=0;j<strs.length;j++){
					if(j==0){
						criteria.createCriteria(strs[j].replaceAll("_cnm", ""),"sb"+i+j);
					}else{
						int k = j-1;
						if(strs[j].indexOf("_cnm")!=-1){
							criteria.createCriteria("sb"+i+k+"." + strs[j].replaceAll("_cnm", ""),"sb"+i+j);
						}else{
							if(strs[j].indexOf(" like ")!=-1){
								nmb1 = strs[j].split(" like ")[1].toString();
								mmp = strs[j].split(" like ")[0];
								cnm = "sb"+i+k;
							}else if(strs[j].indexOf("<>")!=-1){
							    nmb1 = strs[j].split("<>")[1].toString();
                                mmp = strs[j].split("<>")[0];
                                cnm = "sb"+i+k;
							}else{
								nmb = Integer.valueOf(strs[j].split("=")[1]);
								mmp = strs[j].split("=")[0];
								cnm = "sb"+i+k;
							}
						}
				...
				if(str.indexOf(" like ")!=-1){
					criteria.add(buildCriterion(cnm+"."+mmp,nmb1,MatchType.LIKE));
				}else if(str.indexOf("<>")!=-1){
				    criteria.add(buildCriterion(cnm+"."+mmp,nmb1,MatchType.NE));
                }else{
					criteria.add(buildCriterion(cnm+"."+mmp,nmb,MatchType.EQ));
				}
			}else{
				criteria.add(c);
			}
     ...
     * @return
     */
    protected String removeGroups(String qlString) {
	 ...
	 * @param criterions 数量可变的Criterion.
	 */
	public List<T> findjoin(Map<String,String>  map,final Criterion... criterions) {
		return createCriteria( map,criterions).setCacheable(true).list();
		//改变
	}
	 ...
	 * @param criterions 数量可变的Criterion.
	 */
	public Criteria createCriteria(Map<String,String>  map,final Criterion... criterions) {
		Criteria criteria = getSession().createCriteria(entityClass);
		...
		for (String key : map.keySet()) 
			{criteria.createAlias(key, map.get(key),1);  //可以指定使用left out  join..，可以改 
